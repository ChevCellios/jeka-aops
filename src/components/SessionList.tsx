...
export function SessionList({ sessions, analysisProgress, onAnalyze, onDelete, onPlay, onReport, onShare }: Props) {
  return (
        ...
        {sessions.length === 0 ? <Text style={styles.empty}>Još nema snimljenih sesija.</Text> : sessions.map(session => (
          <View key={session.id} style={styles.card}>
            ...
            <View style={styles.actions}>
              ...
              <Action label="Analiziraj" onPress={() => onAnalyze(session)} style={styles.analyze} />
              ...
              <Action label="Obriši" onPress={() => onDelete(session)} style={styles.delete} textStyle={styles.deleteText} />
            </View>
          </View>
        ))}
      </ScrollView>
...
function Action({ label, onPress, style, textStyle }: { label: string; onPress: () => void; style: object; textStyle?: object }) {
  return <Pressable accessibilityRole="button" onPress={onPress} style={[styles.action, style]}><Text style={[styles.actionText, textStyle]}>{label}</Text></Pressable>;
}
